...
def read_and_decode(filename_queue, obs_shape):
	...
	obs = tf.cast(features['obs'], tf.float32) * scale_factor
	next_obs = tf.cast(features['next_obs'], tf.float32)*scale_factor
	...
	with sess.as_default():
		...
		train_summary_op = tf.summary.merge(
			[train_inverse_loss_summ, 
			 ...
			 forward_data_variance_summ,] + train_inverse_separate_summ)
