...
def conv2d(
    ...
    W_shape = list(filter_size + (num_input_channels, num_filters))
    ...
    if pad == "SAME":
        # height = np.ceil((input_shape[1] + stride - 1) / stride)
        # width = np.ceil((input_shape[2] + stride - 1) / stride)
        height = int(np.ceil(input_shape[1]/stride))
        width = int(np.ceil(input_shape[2]/stride))
    else:
        raise ValueError

    output_shape = [None, height, width, num_filters]
    b_shape = output_shape[1:]

    W = weight_variable(W_shape, initializer=W_initializer, name=W_name)
    b = bias_variable(b_shape, initializer=b_initializer, name=b_name)

    output = nonlinearity(tf.nn.conv2d(last_layer, W, (1, stride, stride, 1), pad) + b)
...
def conv_network(input_layer,
    ...
    assert len(conv_filters) == len(conv_filter_sizes) == len(conv_strides) == len(conv_pads)

    last_layer = input_layer
    if len(input_layer_size) == 4:
        last_layer_size = input_layer_size
    elif len(input_layer_size) == 3:
        last_layer_size = [None] + list(input_layer_size)
    else:
        raise ValueError("Invalid input layer size: {}".format(input_layer_size))

    layer = 0
    for num_filters, filter_size, stride, pad in zip(conv_filters, \
                                                     conv_filter_sizes, \
                                                     conv_strides, \
                                                     conv_pads):
        with tf.variable_scope('hidden{0}'.format(layer)) as _:
            pre_nonlin, output_shape = conv2d(
                last_layer,
                last_layer_size,
                num_filters,
                filter_size,
            ...
            if layer == len(conv_filters) - 1:  # Last layer
            ...
            last_layer_size = output_shape
            layer += 1

    # ## TODO: flatten layer and add mlp
    # last_dim = np.prod(last_layer_size[1:])
    # last_layer = tf.flatten(last_layer, [-1, last_dim])

    return last_layer, last_layer_size
