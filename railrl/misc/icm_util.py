...
def analyze_ef_range(env, policy, num_sample=100, data_path='tmp/data'):
	ef_xyz = np.zeros([num_sample, 3])
	obs = env.reset()
	for i in range(num_sample):
		action, _ = policy.get_action(obs)
		next_obs, reward, done, env_info = env.step(action)
		d = env._wrapped_env.wrapped_env.env.env.unwrapped.data
		...
		if done:
			...
			obs = next_obs
	np.save(data_path, ef_xyz)
	print ("Saved end effector data to %s" % data_path)


def analyze_forward_loss(
		_encoder, 
		_forward_model,
		_inverse_model,
		env,
	...
	images = np.zeros([num_sample, 500, 500, 3], dtype='uint8')
	forward_losses = np.zeros(num_sample, dtype='float32')
	boxes_contacts = np.zeros(num_sample, dtype='uint8')
	table_contacts = np.zeros(num_sample, dtype='uint8')
	# states = np.zeros((num_sample, 500, 500, 3), dtyoe='float32')
	...
	for t in range(num_sample):
		...
		action, _ = policy.get_action(obs)
		next_obs, reward, done, env_info = env.step(action)
		forward_loss_result = get_forward_loss([obs], [next_obs], [action])
		...
		images[t] = env.wrapped_env.wrapped_env.env.render(mode='rgb_array')
		# print (forward_loss_result.shape)
	...
	print ("Saved data to {}".format(data_path))
...
def investigate_forward_loss(
	...
	for n in range(num_trajs):
		...
		for t in range(env.wrapped_env._wrapped_env.env.spec.max_episode_steps):
			...
			elif env_infos['contact_table_reward']:
				con_table_obs.append(ob)
				con_table_next_obs.append(next_ob)
				con_actions.append(action)
			else:
	...
	contacts_table_fw_losses = list(get_forward_loss(con_table_obs, con_table_next_obs, con_table_actions))
	non_contacts_fw_losses = list(get_forward_loss(non_con_obs, non_con_next_obs, non_con_actions))
	fw_losses = contacts_fw_losses + contacts_table_fw_losses + non_contacts_fw_losses
	obs = con_obs + con_table_obs + non_con_obs
	next_obs = con_next_obs + con_table_next_obs + non_con_next_obs
	np_obs = np.array(obs)
	...
	for i, ob, next_ob in zip(range(num_top), top_obs, top_next_obs):
		...
		img = img_ob
		scm.imsave(data_path+'obs{}.png'.format(i), img)
		print ('Saved data {}'.format(i))


	np.save(data_path+'con_fw_loss', contacts_fw_losses)
	np.save(data_path+'con_table_fw_loss', contacts_table_fw_losses)
	np.save(data_path+'noncon_fw_loss', non_contacts_fw_losses)
	print ("Saved losses data")

def plot_forward(_encoder, _inverse_model, _forward_model, env, policy, sess, plot_freq=10):
	...
	red_patch = mpatches.Patch(color='red', label='boxes touched')
	blue_patch = mpatches.Patch(color='blue', label='table touched')
	green_patch = mpatches.Patch(color='green', label='nothing touched')
	plt.legend(handles=[red_patch, blue_patch, green_patch])
	plt.ion()
	while True:
		ob = env.reset()
		next_ob = None
		x_nothing, x_table, x_boxes = [], [], []
		y_nothing, y_table, y_boxes = [], [], []
		for t in range(env.wrapped_env._wrapped_env.env.spec.max_episode_steps):
			action, _ = policy.get_action(ob)
			next_ob, reward, done, env_infos = env.step(action)
			# import pdb; pdb.set_trace()
			...
			if env_infos['contact_reward']:
				x_boxes.append(t)
				y_boxes.append(forward_loss)
			elif env_infos['table_contact_reward']:
				x_table.append(t)
				y_table.append(forward_loss)
			else:
				x_nothing.append(t)
				y_nothing.append(forward_loss)
			
			if t % plot_freq == 0:
				ax2.scatter(x_nothing, y_nothing, c='green')
				ax2.scatter(x_table, y_table, c='blue')
				ax2.scatter(x_boxes, y_boxes, c='red')
			plt.pause(0.00001)
			# print ("Should plot")
			plt.show()
...
def compare_envs(name1, name2, num):
	import gym
	import matplotlib.pyplot as plt
	import numpy as np
	import time
	...
	env2.reset()
	# import pdb; pdb.set_trace()
	env1.env.model.data.qpos = env2.env.model.data.qpos
	env1.env.model.data.qvel = env2.env.model.data.qvel
	joint_env1 = []
	joint_env2 = []
	counter = 0
	flag = 1
	while flag:
		...
		if counter > 0:
			time.sleep(0.15)
			counter -= 1
		joint_env1.append(env1.env.model.data.qpos[:4])
		joint_env2.append(env2.env.model.data.qpos[:4])
	joint_env1 = np.array(joint_env1)
	joint_env2 = np.array(joint_env2)
	# f, [ax1, ax2, ax3, ax4] = plt.subplots(4)
	# ax1.plot(joint_env1[:,])
	for i in range(4):
		plt.subplot(2,2,i+1)
		plt.plot(joint_env1[:,i], color = "blue")
		plt.plot(joint_env2[:,i], color = "orange")
	plt.show()