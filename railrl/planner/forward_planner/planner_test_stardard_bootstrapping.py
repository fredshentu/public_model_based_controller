...
scale_factor = np.ones(21)

#Box3dPush-v24 obs scale factor
scale_factor[:7] = np.ones(7)*50
loss_square_scale_factor = 2500
...
if __name__ == "__main__":
	...
	from planner import ClippedSgdForwardModelPlanner, InverseModelPlanner, \
					...
					FastClippedSgdShootingForwardModelPlanner_cumulated_obj, CEMPlanner
	...
	with tf.Session() as sess:
		...
		for j in range(args.num_test):
			...
			for index in index_list:
				...
				elif args.env_name == 'Box3dPush-v2' or args.env_name == 'Box3dPush-v24':
					S_goal_state[4:7] = S_goal_state[4:7]*10
					S_init_state[4:7] = S_init_state[4:7]*10
				...
				for i in range(20):
					if args.shooting:
						...
						if args.variable_horizon:
							action, planner_info = controller.get_action(obs, S_goal_state, steps = max(15-i,1))
						else:
							action, planner_info = controller.get_action(obs*scale_factor, S_goal_state*scale_factor, steps = args.horizon)
					else:
						action, planner_info = controller.get_action(obs, S_goal_state)
					...
					# print(action)
					arm_predicted_l2_dis = planner_info['arm_loss']/loss_square_scale_factor
					box_predicted_l2_dis = planner_info['box_loss']/loss_square_scale_factor
					...
					if args.vis:
						old_state = obs.copy()
						if args.env_name == 'Box3dPush-v1' or args.env_name == 'Box3dPush-v2' or args.env_name == "Box3dPush-v24":
							old_state[4:7] = old_state[4:7]/10.0
						
						im_list = []
						# print(forward_model_output_list)
						for s in forward_model_output_list:
							s = s.flatten()/scale_factor
							# print(s)
							s[4:7] = s[4:7]/10.0
							env_set_state(env, s)
